...
def cross(
    ...
    randint = np.hstack([np.arange(0, Is[n + 1], int(.975*Is[n + 1])/max(Rs))[:max(Rs)].reshape((max(Rs), 1))  for n in range(N - 1)] + [np.zeros([max(Rs), 1], dtype=int)])
    ...
    for i in range(max_iter):
        ...
        if converged:
            ...
            extra = np.hstack([np.arange(0, Is[n + 1], int(.975*Is[n + 1])/max(newRs))[:max(newRs)].reshape((max(Rs), 1))  for n in range(N - 1)] + [np.zeros([max(newRs), 1], dtype=int)])
            for n in range(N - 1):
                if newRs[n + 1] > Rs[n + 1]:
